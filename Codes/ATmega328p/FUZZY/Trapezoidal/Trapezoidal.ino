...
void loop() {
  ...
  if (tempo - ultimoTempo >= periodo || ultimoTempo == 0) {                   // SE já se passou pelo menos um "período"
    ...
    for (int MFsIN1 = 0; MFsIN1 < numMFsIn1; MFsIN1++) {
      miIn1[MFsIN1] = trapezoidal(paramtersIn1[3 * MFsIN1], paramtersIn1[3 * MFsIN1 + 1],
                                  paramtersIn1[3 * MFsIN1 + 2], paramtersIn1[3 * MFsIN1 + 3], erro);
    }
    // Raliza a fuzzyficação da variável "erro", entrada 1 do Fuzzy-PI
    for (int MFsIN2 = 0; MFsIN2 < numMFsIn2; MFsIN2++) {
      miIn2[MFsIN2] = trapezoidal(paramtersIn2[3 * MFsIN2], paramtersIn2[3 * MFsIN2 + 1],
                                  paramtersIn2[3 * MFsIN2 + 2], paramtersIn2[3 * MFsIN2 + 3], varErro);
    }
    // ETAPA DE INFERÊNCIA
    int contOut = 0;
...
float trapezoidal(float a, float b, float c, float d, float x) {  // Função Matemática Trapezoidal
  float mi = 0;                                                   // Define mi = 0
  if (x >= a && x < b)                                            // SE x estiver entre a e b
    mi = (x - a) / (b - a);                                       // Usa a eq da reta de subida
  else if (x >= b && x < c)
    mi = 1.0;  // Se estiver entre B e C, mi =1
  else if (x >= c && x <= d)
    mi = (d - x) / (d - c);  // Usa a eq da reta de descida
